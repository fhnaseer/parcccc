...
public class TripContentProvider extends ContentProvider {
    ...
    private static final int ONE_TRIP = 1; // manipulate one contact
    private static final int TRIPS = 2; // manipulate contacts table

    // static block to configure this ContentProvider's UriMatcher
    static {
        // Uri for Trip with the specified id (#)
        uriMatcher.addURI(DatabaseDescription.TRIP_AUTHORITY,
    ...
    public Cursor query(Uri uri, String[] projection,
        ...
        switch (uriMatcher.match(uri)) {
            case ONE_TRIP: // contact with specified id will be selected
                queryBuilder.appendWhere(
                        Trip._ID + "=" + uri.getLastPathSegment());
                break;
            case TRIPS: // all contacts will be selected
                break;
            default:
        ...
        switch (uriMatcher.match(uri)) {
            case ONE_TRIP:
                ...
                numberOfRowsUpdated = dbHelper.getWritableDatabase().update(
                        Trip.TABLE_NAME, values, Trip._ID + "=" + id,
                        selectionArgs);
                break;
            default:
                throw new UnsupportedOperationException(
        ...
        switch (uriMatcher.match(uri)) {
            case ONE_TRIP:
                ...
                numberOfRowsDeleted = dbHelper.getWritableDatabase().delete(
                        Trip.TABLE_NAME, Trip._ID + "=" + id, selectionArgs);
                break;
            default:
                throw new UnsupportedOperationException(
